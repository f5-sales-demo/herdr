...
protocol-22
`native_launch` v4 contract: it measures a canonical session directory and
...
| Protocol-26 native launch / consumer | Installed Herdr protocol >=26 for `execution.resume` v4 native-launch binding with `read_interactions`, plus `agent_turn_journal`, consumer enabled, and persisted cursor/result-digest receipt | Manager boundary code plus synthetic fixture only | Blocked pending installed capability |
...
dedicated disposable runtime; query real protocol-26 native-launch and journal records; and
