...
impl PaneRuntime {
    ...
    pub fn spawn_argv_command(
        ...
        launch_env: &PaneLaunchEnv,
        agent_detection: AgentDetection,
        execution_manager: Option<crate::execution::ExecutionManager>,
        scrollback_limit_bytes: usize,
        host_terminal_theme: crate::terminal_theme::TerminalTheme,
        host_terminal_appearance: Option<crate::terminal_theme::HostAppearance>,
        events: mpsc::Sender<AppEvent>,
    ...
    fn spawn_command_builder(
        ...
        let child_killer = Arc::new(Mutex::new(spawned.child.clone_killer()));
        let content_seq = Arc::new(AtomicU64::new(0));
        let detection_content_seq = Arc::new(AtomicU64::new(0));
        let full_lifecycle_authority_active = Arc::new(AtomicBool::new(false));
        {
        ...
        let io = {
            ...
            let execution_manager_for_output = execution_manager.clone();
            let on_read = Box::new(move |bytes: &[u8]| {
                if let Some(manager) = &execution_manager_for_output {
                    manager.observe_visible_output(pane_id.raw(), bytes);
                }
                let _content_write_guard = match content_write_lock.lock() {
                    Ok(guard) => guard,
                    Err(poisoned) => poisoned.into_inner(),
