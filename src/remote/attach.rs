...
fn remote_release_asset(asset_key: &str) -> io::Result<RemoteReleaseAsset> {
    if crate::build_info::is_preview() {
        return Err(io::Error::other(format!(
            "preview remote bootstrap is unavailable until the maintained fork has a fork-owned preview channel; set {REMOTE_BINARY_ENV_VAR}=target/release/herdr or install a matching Herdr on the remote host manually"
        )));
    }
