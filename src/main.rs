...
# This maintained fork supports only the stable update channel.
# channel = "stable"

# Check the fork-owned stable manifest for new Herdr versions in the background.
# version_check = true

# Check the fork-owned catalog for remote agent-detection manifest updates in the background.
# manifest_check = true
...
fn main() -> io::Result<()> {
    ...
    if args.iter().any(|a| a == "--help" || a == "-h") {
        ...
        println!("Home:   https://github.com/f5-sales-demo/herdr");
        println!();
