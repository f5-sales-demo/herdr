...
use crate::api::schema::{
    ...
    PaneRenameParams, PaneReportAgentHeartbeatParams, PaneReportAgentParams,
...
impl App {
    pub(super) fn handle_pane_report_agent_heartbeat(
        &mut self,
        id: String,
        params: PaneReportAgentHeartbeatParams,
    ) -> String {
        ...
        self.handle_internal_event(crate::events::AppEvent::AgentHeartbeatReported {
            pane_id,
...
mod tests {
    ...
    #[tokio::test]
    async fn api_pane_send_keys_accepts_editing_and_paging_keys() {
        ...
        let response = app.handle_api_request(crate::api::schema::Request {
            ...
            method: crate::api::schema::Method::PaneSendKeys(PaneSendKeysParams {
                pane_id,
                keys: vec!["delete".into(), "pageup".into(), "pagedown".into()],
            }),
        });
        ...
        assert_eq!(
            rx.try_recv().unwrap(),
            bytes::Bytes::from_static(b"\x1b[3~")
        );
        assert_eq!(
            rx.try_recv().unwrap(),
            bytes::Bytes::from_static(b"\x1b[5~")
        );
        assert_eq!(
            rx.try_recv().unwrap(),
            bytes::Bytes::from_static(b"\x1b[6~")
        );
        assert!(rx.try_recv().is_err());
    }
