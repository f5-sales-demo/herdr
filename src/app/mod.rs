...
impl App {
    ...
    pub fn new(
        ...
        Self::new_with_runtime_state(
            config,
            policy,
            config_diagnostic,
            api_rx,
            event_hub,
            crate::execution::SharedRuntimeState::load(),
        )
    }

    pub(crate) fn new_with_runtime_state(
        config: &Config,
        policy: AppPolicy,
        config_diagnostic: Option<String>,
        api_rx: tokio::sync::mpsc::UnboundedReceiver<crate::api::ApiRequestMessage>,
        ...
        let (workspaces, active, selected) = if let Some(snap) = snapshot {
            let history = config
                ...
                .flatten();
            let restore_options = crate::persist::RestoreOptions::new(
                24,
                80,
            ...
            let restore_runtime = crate::persist::RestoreRuntime::new(
                event_tx.clone(),
                render_notify.clone(),
                render_dirty.clone(),
            );
            let (ws, terminals, terminal_runtimes) =
                crate::persist::restore(&snap, history.as_ref(), &restore_options, restore_runtime);
    ...
    pub fn new_from_handoff(
        ...
        let mut app = Self::new_with_runtime_state(
            config,
        ...
        let (workspaces, terminals, runtimes) =
            crate::persist::restore_handoff(snapshot, &restore_options, imports, restore_runtime)?;
        let pane_id_aliases = crate::persist::handoff_pane_aliases(snapshot, &workspaces);
        app.runtime_state
            .executions
            .rebind_handoff_panes(workspaces.iter().flat_map(|workspace| {
                workspace
                    .public_pane_numbers
                    .iter()
                    .map(|(pane_id, number)| {
