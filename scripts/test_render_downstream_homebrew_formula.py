...
if __package__:
    from scripts.render_downstream_homebrew_formula import TARGETS, load_checksums, render_formula
else:
    from render_downstream_homebrew_formula import TARGETS, load_checksums, render_formula
    ...
    def test_checksum_files_must_name_the_expected_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            for target, checksum in self.checksums.items():
                ...
                (root / f"{artifact}.sha256").write_text(
                    f"{checksum}  {artifact}\n", encoding="utf-8"
                )
