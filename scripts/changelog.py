...
DEFAULT_LIVE_MANIFEST_URL = (
    "https://raw.githubusercontent.com/f5-sales-demo/herdr/build-xcsh/distribution/latest.json"
)

SECTION_RE = re.compile(r"^##\s+(?:\[(?P<bracketed>[^\]]+)\]|(?P<plain>.+?))\s*$", re.MULTILINE)
VERSION_WITH_DATE_RE = re.compile(r"^(?P<version>.+?)\s+-\s+\d{4}-\d{2}-\d{2}$")
DEFAULT_RELEASE_REPO = "f5-sales-demo/herdr"
...
def latest_release_assets(repo: str = DEFAULT_RELEASE_REPO) -> dict[str, str]:
    return {
        target: f"https://github.com/{repo}/releases/latest/download/{asset_name}"
...
def manifest_from_release_payload(
    ...
    for target, asset_name in EXPECTED_ASSET_NAMES.items():
        ...
        published_url = str(asset.get("url") or "").strip()
        ...
        manifest_assets[target] = latest_release_assets(repo)[target]
