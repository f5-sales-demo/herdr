...
if ($channelWasExplicit -and $Channel -ne "stable") {
    Write-Error "Invalid Herdr channel '$Channel'. This maintained fork currently supports only stable."
    exit 1
}
...
if ($useLocalPackage) {
    ...
    if (-not $channelWasExplicit) {
        if (-not [string]::IsNullOrWhiteSpace($channelHerdr) -and $null -eq $existingHerdrKind) {
            throw "Refusing to run unrecognized Herdr command at $channelHerdr to detect its update channel. Rerun with -Channel stable."
        }
        if (-not [string]::IsNullOrWhiteSpace($channelHerdr)) {
            ...
            if ($LASTEXITCODE -ne 0 -or $detectedChannel -notin @("stable", "preview")) {
                throw "Could not determine the existing Herdr update channel. Rerun with -Channel stable."
            }
            if ($detectedChannel -eq "preview") {
                throw "Preview installation is unavailable until the maintained fork has a fork-owned preview channel. Rerun with -Channel stable."
            }
    ...
    if ([string]::IsNullOrWhiteSpace($ManifestUrl)) {
        $ManifestUrl = "https://raw.githubusercontent.com/f5-sales-demo/herdr/build-xcsh/distribution/latest.json"
    }
    ...
    if ($null -ne $manifestChannelProperty -and [string]$manifestChannelProperty.Value -eq "preview") {
        throw "Preview installation is unavailable until the maintained fork has a fork-owned preview channel."
    }
