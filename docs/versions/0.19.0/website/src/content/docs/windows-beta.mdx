...
Windows builds are available through the fork-owned stable update channel. Preview installation and self-update fail closed until the maintained fork has its own preview infrastructure. If an older installation is configured for preview, run `herdr channel set stable` before updating.
