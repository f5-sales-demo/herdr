...
For Linux and macOS hosts, Herdr can copy the current local binary when the platforms match. For Homebrew and Nix installs, when the platforms differ, or for Windows hosts, it downloads the matching release asset for the current client version from the fork-owned `distribution/latest.json` manifest on `build-xcsh`.
