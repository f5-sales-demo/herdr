...
| xcsh | bundled reporter (ships with xcsh); otherwise screen manifest | state and session |
...
Bundled manifests live inside Herdr. This maintained fork also checks its fork-owned distribution catalog for remote manifest updates and applies valid per-agent rule updates automatically without requiring a Herdr restart. Remote manifests are stored in Herdr's state directory. Set `[update] manifest_check = false` to disable background remote manifest checks.
