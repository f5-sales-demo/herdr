...
## Native XCSH resume binding

`execution.resume` is a native lifecycle protocol, not a shell-command
shortcut. Supply the exact canonical XCSH `SessionHeader` ID and an absolute
`xcsh_executable` path. Herdr canonicalizes and hashes that executable before
the durable generation claim, stores the binding in the execution receipt, and
checks it again immediately before launch. A retry must provide the same
binding; a relative, unavailable, or ordinarily replaced executable fails
closed. This is a measured owner-controlled-artifact boundary, not an atomic
...
```json
{"id":"resume-1","method":"execution.resume","params":{"execution_id":"task-42","generation":1,"session_id":"0123abcd4567ef89","xcsh_executable":"/opt/xcsh/bin/xcsh","text":"continue","cwd":"/work/project"}}
```
