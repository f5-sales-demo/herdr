...
Pass extra CLI args to a supported agent's native resume command on session restore:
...
Keys are canonical agent IDs. Args are appended to the native resume command only; they do not affect normal launches. Only agents with native resume support take effect.
